...
baseline += " & (HLT_Tau3Mu_Mu5_Mu1_TkMu1_IsoTau10_Charge1_matched || HLT_Tau3Mu_Mu7_Mu1_TkMu1_IsoTau15_Charge1_matched)"
...
baseline += " & (   (mu1_refit_muonid_medium & mu2_refit_muonid_medium & mu3_refit_muonid_medium) )"
...
getattr(wspace, 'import')(ROOT.RooRealVar('cand_refit_tau_pt'   , 'cand_refit_tau_pt'   , 0, 300))
...
getattr(wspace, 'import')(ROOT.RooRealVar('mu1_refit_pt', 'mu1_refit_pt', 0 , 200))
getattr(wspace, 'import')(ROOT.RooRealVar('mu2_refit_pt', 'mu2_refit_pt', 0 , 200))
getattr(wspace, 'import')(ROOT.RooRealVar('mu3_refit_pt', 'mu3_refit_pt', 0 , 200))
...
getattr(wspace, 'import')(ROOT.RooRealVar('mu1_refit_muonid_soft', 'mu1_refit_muonid_soft', 0 , 1))
getattr(wspace, 'import')(ROOT.RooRealVar('mu2_refit_muonid_soft', 'mu2_refit_muonid_soft', 0 , 1))
getattr(wspace, 'import')(ROOT.RooRealVar('mu3_refit_muonid_soft', 'mu3_refit_muonid_soft', 0 , 1))
getattr(wspace, 'import')(ROOT.RooRealVar('mu1_refit_muonid_medium', 'mu1_refit_muonid_medium', 0 , 1))
getattr(wspace, 'import')(ROOT.RooRealVar('mu2_refit_muonid_medium', 'mu2_refit_muonid_medium', 0 , 1))
getattr(wspace, 'import')(ROOT.RooRealVar('mu3_refit_muonid_medium', 'mu3_refit_muonid_medium', 0 , 1))
...
getattr(wspace, 'import')(ROOT.RooRealVar('HLT_Tau3Mu_Mu7_Mu1_TkMu1_IsoTau15_Charge1_matched', 'HLT_Tau3Mu_Mu7_Mu1_TkMu1_IsoTau15_Charge1_matched', 0 , 1))
...
poly = ROOT.RooChebychev  ('bkg_poly', 'bkg_poly', wspace.var('cand_refit_tau_mass'), ROOT.RooArgList(poly1, poly2))
...
if args.discrete_profiling:
    ...
    background = ROOT.RooAddPdf('bkg', '', ROOT.RooArgList(expo), ROOT.RooArgList(nbkg))
...
low17_sel = category_selection.format(BDT = 0.991, IWP = 0.000, OWP = 0.007, YAR = 17) ## 0.996
mid17_sel = category_selection.format(BDT = 0.994, IWP = 0.007, OWP = 0.012, YAR = 17) ## 0.996
hig17_sel = category_selection.format(BDT = 0.992, IWP = 0.012, OWP = 10000, YAR = 17) ## 0.992

low18_sel = category_selection.format(BDT = 0.995, IWP = 0.000, OWP = 0.007, YAR = 18) ## 0.996
mid18_sel = category_selection.format(BDT = 0.998, IWP = 0.007, OWP = 0.012, YAR = 18) ## 0.995
hig18_sel = category_selection.format(BDT = 0.994, IWP = 0.012, OWP = 10000, YAR = 18) ## 0.993

low17 = Category(blind = not args.unblind, name = 'A17', sig_norm = MC_NORM17, working_point = 'bdt{BDT}'.format(BDT = 0.991), selection = low17_sel, wspace = wspace_A17, sigma = 0.02, dp = args.discrete_profiling)#, Z_frac = 0.117)
mid17 = Category(blind = not args.unblind, name = 'B17', sig_norm = MC_NORM17, working_point = 'bdt{BDT}'.format(BDT = 0.994), selection = mid17_sel, wspace = wspace_B17, sigma = 0.06, dp = args.discrete_profiling)#, Z_frac = 0.085)
hig17 = Category(blind = not args.unblind, name = 'C17', sig_norm = MC_NORM17, working_point = 'bdt{BDT}'.format(BDT = 0.992), selection = hig17_sel, wspace = wspace_C17, sigma = 0.02, dp = args.discrete_profiling)#, Z_frac = 0.090)

low18 = Category(blind = not args.unblind, name = 'A18', sig_norm = MC_NORM18, working_point = 'bdt{BDT}'.format(BDT = 0.995), selection = low18_sel, wspace = wspace_A18, sigma = 0.02, dp = args.discrete_profiling)#, Z_frac = 0.052)
mid18 = Category(blind = not args.unblind, name = 'B18', sig_norm = MC_NORM18, working_point = 'bdt{BDT}'.format(BDT = 0.998), selection = mid18_sel, wspace = wspace_B18, sigma = 0.06, dp = args.discrete_profiling)#, Z_frac = 0.043)
hig18 = Category(blind = not args.unblind, name = 'C18', sig_norm = MC_NORM18, working_point = 'bdt{BDT}'.format(BDT = 0.994), selection = hig18_sel, wspace = wspace_C18, sigma = 0.02, dp = args.discrete_profiling)#, Z_frac = 0.048)
## SYSTEMATICS
##
Z_frac = 0#0.84
W_frac = 1. - Z_frac
...
#cfg.run_combine(ntoys = 500, grid = args.quantile, asymptotic = args.asymptotic, cl = args.cl)
